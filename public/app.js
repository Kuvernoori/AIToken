const tokenAddress = "0x8B737A571c4121345117Bbe8a93326FC18F8863A"; // Адрес вашего контракта
  ...
  // Слушаем изменение аккаунтов в MetaMask и обновляем баланс
  ...
  window.onload = async () => {
      if (window.ethereum) {
          ...
          if (accounts.length > 0) {
              account = accounts[0];
              document.getElementById('walletAddress').textContent = `Wallet Address: ${account}`;
              await updateTokenBalance(); // Обновляем баланс при загрузке страницы
          }
      }
  };
    ...
    async function createModelListing(event) {
      ...
      const modelUrl = document.getElementById("model-url").value; // Получаем URL модели
      ...
      const response = await fetch("/api/createModel", {
        ...
        body: JSON.stringify({
          ...
          price: modelPrice,
          url: modelUrl, // Отправляем URL вместо файла
    ...
    async function loadModels() {
      ...
      models.forEach(model => {
        ...
        modelItem.classList.add("model-item");
            // Принудительное центрирование и стилизация
    ...
    async function buyModel(sellerAddress, amount) {
      try {
          ...
          // Отправка транзакции с обработкой событий
